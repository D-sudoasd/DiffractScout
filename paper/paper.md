...
Powder X-ray diffraction studies often begin with a reference-building task: researchers assemble plausible crystal structures, calculate where their diffraction peaks should occur, and retain enough context to judge whether those references are suitable for a given experiment. `DiffractScout` is an open-source Python package that makes this task reproducible. It accepts an alloy grade, chemical system, Materials Project identifier, or local Crystallographic Information File (CIF); discovers and ranks candidate phases when requested; validates the acquired structures; calculates indexed theoretical powder reflections; and optionally associates each reflection with the Young's modulus along its reciprocal-lattice normal when a compatible elastic tensor is available. Every output remains linked to the exact input structure, radiation settings, tensor source and coordinate frame, software versions, warnings, and file hashes. The result is a self-contained evidence bundle containing machine-readable provenance, tabular references, a spreadsheet, structured diagnostics, and a strict SHA-256 manifest. The package provides a command-line interface, Python API, desktop interface, and a deterministic offline verification example.
...
The 68-test suite covers composition parsing, candidate selection, CIF and tensor validation, systematic absences, structure factors, intensity definitions, coordinate-frame boundaries, resource limits, transactional replacement, spreadsheet safety, and manifest tamper detection. A separate packaged benchmark performs 45 closed-form checks across simple-cubic, BCC, FCC, NaCl, and cubic directional-elasticity cases and writes a self-verifying evidence bundle. Continuous integration targets Python 3.10--3.13, Windows, macOS, a Linux graphical-interface smoke test, wheel installation, and Open Journals draft compilation. These materials let reviewers reproduce the declared numerical and provenance contracts without database credentials. Experimental accuracy and external adoption remain separate evidence. They are tracked through versioned validation cases and a machine-readable public evidence ledger, and the final submission will include only claims supported by those records.
